<div class="modal" id="modalSubirRayos" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<form id="frm-subir-rayos" method="POST" action="/subir-rayosx" enctype="multipart/form-data">
		...
		<?php foreach ($rayos->result() as $rayo):?>
			<div class="col-md-3" style="margin-bottom:12px;">
				...
				<div class="caption">
					...
					<p><?=@$rayo->fecha?></p>
					<p>
						<a href="/rayosx/rayosx/download/<?=$rayo->nombre.$rayo->tipo.'/'.$rayo->historiaclinicaid?>" class="btn btn-primary" role="button"><i class="glyphicon glyphicon-download-alt"></i></a>
						<a id="eliminar-rayo" url="/eliminar-rayosx/<?=$rayo->rayosxid?>/<?=$rayo->historiaclinicaid?>/<?=@$rayo->nombre.@$rayo->tipo?>" class="btn btn-primary" role="button"><i class="glyphicon glyphicon-trash"></i></a>
					</p>
				</div>
			</div>
		<?php endforeach;?>
